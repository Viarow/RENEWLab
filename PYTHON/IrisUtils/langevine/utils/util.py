...
def processData(ofdmSymb, subcarrier, int_min, int_max, pilot_sc_ind, pilot_sc_val, csi_f, ul_syms_f, tx_syms_f, config):

    pilotIndex = pilot_sc_ind
    pilotValue = pilot_sc_val

    HPilot = csi_f[int_min:int_max, :, :, pilotIndex]
    H = csi_f[int_min:int_max, :, :, subcarrier]
    yPilot = ul_syms_f[int_min:int_max, :, ofdmSymb, pilotIndex]
    yPilot = np.transpose(yPilot, (2, 0, 1))
    y = ul_syms_f[int_min:int_max, :, ofdmSymb, subcarrier]
        
    xPilot = tx_syms_f[int_min:int_max, :, ofdmSymb, pilotIndex]
    x = tx_syms_f[int_min:int_max, :, ofdmSymb, subcarrier]
